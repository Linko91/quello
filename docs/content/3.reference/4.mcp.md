...
`server.handle(message)` takes one decoded JSON-RPC message and returns the response, or `null` for
a notification — so any transport can sit in front of it.
...
stdio transport, JSON-RPC 2.0 in newline-delimited JSON. Written against the `2025-06-18` revision
and answering to `2024-11-05` through `2025-11-25`.

Zero dependencies beyond quello's own packages: the wire format is a line of JSON, small enough to
own outright, and the rest of quello hand-rolls its HTTP for the same reason.
