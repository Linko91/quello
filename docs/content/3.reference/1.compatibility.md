...
The repository keeps both halves under test by hand: the `react18` and `react19` playgrounds are the
same app on the same Vite and the same `@vitejs/plugin-react`, differing only in the React version,
so picking the same element in each shows the line appear and disappear.
::

::note{title="² On Next, the file follows the bundler"}
Both Next majors read owner stacks, but what a frame names depends on what compiled it. **webpack**
names the source — `webpack-internal:///(rsc)/./app/page.tsx` normalises back to `app/page.tsx`.
**Turbopack**, the default from Next 16 on, names the chunk that happens to hold the module,
`/_next/static/chunks/_0ltyzvd._.js`. That is a real file and exactly the wrong one, so quello drops
the frame rather than pointing an agent at a bundle: a Next 16 pick carries the component, the
selector and the DOM path, without a file.

The `next15` and `next16` playgrounds run one each.
::
