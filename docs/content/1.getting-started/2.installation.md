...
## Optional: reach the picks over MCP

The four routes above put the picker in your page. How your *agent* then reads the picks is a
separate question, and it has two answers: the agent file quello writes on first run, or
[`@quello/mcp`](/reference/mcp) — the picks as tools, for editors that read no agent file.

```bash [Terminal]
claude mcp add quello -- npx -y @quello/mcp
```

It is orthogonal to your builder, read-only, and writes nothing into your project.
