...
export function markSvg(height = 21): string {
  ...
  <circle cx="8.6" cy="8.6" r="6" stroke="currentColor" stroke-width="2.9"/>
  <circle cx="8.6" cy="8.6" r="2" fill="currentColor"/>
  <path d="M14.6 8.6 V21.9" stroke="currentColor" stroke-width="2.9" stroke-linecap="square"/>
</svg>`
}

/**
 * The wordmark: the same "q", with "uello" built on its grid — one stroke weight
 * (2.9), one x-height (the bowl's diameter), square caps and round joins
 * throughout, so the letters read as a family rather than as a mark glued to a font.
 */
export function logoSvg(height = 24): string {
  ...
  <g stroke="currentColor" stroke-width="2.9" stroke-linecap="square" stroke-linejoin="round">
