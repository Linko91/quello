...
> **MVP status** — dev mode only. Vite, webpack, Next, or no bundler at all: fourteen playgrounds
...
Every quello package is a development dependency. The plugins do nothing in a production build,
`<Quello />` renders `null` there and its route answers `404` — nothing here reaches your users,
...
apply exactly as they would to an injected tag. In a production build it resolves to an empty
module, so the import still bundles and ships nothing.
...
- **Dev only.** A production build gets no script tag, no agent file and no quello code in the
  bundle. The plugin's only build-time job is resolving `virtual:quello` to an empty module, which
  it has to do because the dev guard around that import is a runtime check — Rolldown, which Vite 8
  builds on, fails a build on an id nothing resolves.
...
of the work now is upkeep: fourteen playgrounds that have to keep agreeing with the
