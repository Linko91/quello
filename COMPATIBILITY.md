...
- **Nuxt** works the same on both majors and a pick knows the same things, but Nuxt 4 makes `app/`
  the Vite root, so `.quello/` and `AGENTS.md` land inside `app/` rather than beside
  `package.json`. Both are exercised — [`nuxt3`](playgrounds/nuxt3) and
  [`nuxt4`](playgrounds/nuxt4) — and the one-line way to move them back is in
  [the Nuxt guide](https://quello-docs.vercel.app/guides/nuxt).
